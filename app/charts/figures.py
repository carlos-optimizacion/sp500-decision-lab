...
from .theme import AXIS, BLUE, BLUE_LIGHT, GOLD, INK, MUTED, ORANGE, PANEL, REGIME_COLORS, axis, layout
...
def market_history_figure(
    ...
    if use_candles and not adjusted.empty:
        figure.add_trace(
            go.Candlestick(
                ...
                decreasing={"line": {"color": ORANGE, "width": 1}, "fillcolor": "#7C3A2A"},
                whiskerwidth=0.35,
            ),
            row=1,
            col=1,
    ...
    figure.update_xaxes(showgrid=False, linecolor=AXIS, fixedrange=True, row=2, col=1)
    figure.update_xaxes(fixedrange=True, row=1, col=1)
    ...
    figure = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "borderwidth": 0,
                "steps": [
                    {"range": [0, 40], "color": "#172033"},
                    {"range": [40, 60], "color": "#1E293B"},
                    {"range": [60, 80], "color": "#22345D"},
                    {"range": [80, 100], "color": "#294879"},
                ],
                "threshold": {"line": {"color": GOLD, "width": 3}, "thickness": 0.8, "value": 60},
            },
        )
    )
    figure.update_layout(height=300, margin={"l": 30, "r": 30, "t": 50, "b": 18}, paper_bgcolor=PANEL)
    return figure
...
def conditions_figure(row: pd.Series) -> go.Figure:
    ...
    figure.update_yaxes(autorange="reversed", showgrid=False, linecolor=AXIS, fixedrange=True)
...
def equity_figure(daily: pd.DataFrame) -> go.Figure:
    ...
    figure.update_xaxes(showgrid=False, linecolor=AXIS, fixedrange=True)
...
def annual_returns_figure(annual: pd.DataFrame) -> go.Figure:
    ...
    figure.update_xaxes(title="Año", showgrid=False, linecolor=AXIS, fixedrange=True)
